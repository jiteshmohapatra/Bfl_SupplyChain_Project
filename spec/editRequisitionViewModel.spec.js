...
describe('requisition view model', () => {
  it('should save data to server', () => {
    const originId = '2';
    const dateRequested = '11/12/2012';
    const requestedDeliveryDate = '11/13/2012';
    const requestedById = '23';
    const recipientProgram = 'test';
    const requisitionItem1 = {
      productId: 'prod1',
      quantity: 300,
      version: 1,
      comment: 'my comment1',
    ...
    const viewModel = new openboxes.requisition.EditRequisitionViewModel(requisition);
    const formElement = {
      action: 'testAction',
    };
    viewModel.save(formElement);
    const jsonSent = JSON.parse(ajaxOptions.data);
  ...
  it('should convert null to empty string when saving data to server', () => {
