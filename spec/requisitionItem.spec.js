describe('requisitionItem', () => {
  it('should get quantity picked, remaining and status from requisition item', () => {
    const requisitionItem = new openboxes.requisition.RequisitionItem({ id: '1234', quantity: 5000 });
    // expect(requisitionItem.status()).toBe("Incomplete");
    const picklistItem1 = new openboxes.requisition.PicklistItem({
      quantity: 1000,
      quantityOnHand: 2000,
    });
    ...
    const picklistItem2 = new openboxes.requisition.PicklistItem({
      quantity: 4000,
      quantityOnHand: 4000,
    });
