const app = angular.module('pivotApp', ['googlechart']).controller('mainController', ['$scope', '$http', function ($scope, $http) {
  $scope.results = [];
  ...
  $scope.init = function () {
    ...
    $http.jsonp('/inventory/calculateQuantityOnHandByProduct?callback=JSON_CALLBACK').success((data) => {
      console.log(data);
      ...
      angular.forEach(data.items, (value, index) => {
        let exists = false;
        angular.forEach($scope.availableStatuses, (availableStatus, index) => {
