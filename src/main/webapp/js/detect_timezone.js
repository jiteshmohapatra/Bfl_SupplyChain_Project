 ...
 */
const jzTimezoneDetector = new Object();
...
jzTimezoneDetector.TimeZone.prototype.ambiguity_check = function () {
  const local_ambiguity_list = jzTimezoneDetector.olson.ambiguity_list[this.olson_tz];

  if (typeof (local_ambiguity_list) === 'undefined') {
    return;
  }

  const { length } = local_ambiguity_list;

  for (let i = 0; i < length; i++) {
    const tz = local_ambiguity_list[i];
...
jzTimezoneDetector.date_is_dst = function (date) {
  const base_offset = ((date.getMonth() > 5 ? jzTimezoneDetector.get_june_offset() : jzTimezoneDetector.get_january_offset()));

  const date_offset = jzTimezoneDetector.get_date_offset(date);
...
jzTimezoneDetector.get_timezone_info = function () {
  const january_offset = jzTimezoneDetector.get_january_offset();

  const june_offset = jzTimezoneDetector.get_june_offset();

  const diff = january_offset - june_offset;
...
jzTimezoneDetector.determine_timezone = function () {
  const timezone_key_info = jzTimezoneDetector.get_timezone_info();

  let hemisphere_suffix = '';

  if (timezone_key_info.hemisphere == jzTimezoneDetector.HEMISPHERE_SOUTH) {
    hemisphere_suffix = ',s';
  }

  const tz_key = `${timezone_key_info.utc_offset},${timezone_key_info.dst}${hemisphere_suffix}`;
