...
(function () {
  let $; let PivotData; let addSeparators; let aggregatorTemplates; let aggregators; let convertToArray; let dayNames; let deriveAttributes; let derivers; let forEachRecord; let getPivotData; let mthNames; let naturalSort; let numberFormat; let pivotTableRenderer; let renderers; let spanSize; let zeroPad;
  const __slice = [].slice;
  ...
  aggregatorTemplates = {
    ...
    sumOverSumBound80(sigfig, scaler, upper) {
      if (sigfig == null) {
        sigfig = 3;
      }
      if (scaler == null) {
    ...
    fractionOf(wrapped, type) {
      if (type == null) {
        type = 'total';
      }
      return function () {
        let x;
        x = arguments.length >= 1 ? __slice.call(arguments, 0) : [];
    ...
    l10nWrapper(wrapped, formatter, labelFn) {
      return function () {
        let x;
        x = arguments.length >= 1 ? __slice.call(arguments, 0) : [];
        return function (data, rowKey, colKey) {
  ...
  naturalSort = function (as, bs) {
    let a; let a1; let b; let b1; let rd; let rx; let
      rz;
    rx = /(\d+)|(\D+)/g;
    rd = /\d/;
    rz = /^0/;
  ...
  forEachRecord = function (input, derivedAttributes, f) {
    let addRecord; let compactRecord; let i; let j; let k; let record; let tblCols; let _i; let _len; let _ref; let _results; let
      _results1;
  ...
  PivotData = (function () {
    ...
    PivotData.prototype.processRecord = function (record) {
      let colKey; let flatColKey; let flatRowKey; let rowKey; let
        x;
      colKey = (function () {
  ...
  spanSize = function (arr, i, j) {
    let len; let noDraw; let stop; let x; let _i; let
      _j;
    if (i !== 0) {
      noDraw = true;
      for (x = _i = 0; j >= 0 ? _i <= j : _i >= j; x = j >= 0 ? ++_i : --_i) {
    ...
    while (i + len < arr.length) {
      stop = false;
      for (x = _j = 0; j >= 0 ? _j <= j : _j >= j; x = j >= 0 ? ++_j : --_j) {
  ...
  pivotTableRenderer = function (pivotData, opts) {
    let aggregator; let c; let colAttrs; let colKey; let colKeys; let defaults; let i; let j; let r; let result; let rowAttrs; let rowKey; let rowKeys; let th; let totalAggregator; let tr; let txt; let val; let
      x;
    defaults = {
      localeStrings: {
        totals: 'Totals',
  ...
  $.fn.pivot = function (input, opts) {
    let defaults; let e; let pivotData; let
      result;
    defaults = {
      cols: [],
      rows: [],
  ...
  $.fn.pivotUI = function (input, inputOpts, overwrite) {
    let aggregator; let axisValues; let c; let colList; let defaults; let e; let existingOpts; let i; let k; let opts; let pivotTable; let refresh; let refreshDelayed; let renderer; let rendererControl; let shownAttributes; let tblCols; let tr1; let tr2; let uiTable; let x; let _fn; let _i; let _j; let _k; let _l; let _len; let _len1; let _len2; let _len3; let _ref; let _ref1; let _ref2; let _ref3; let _ref4; let _ref5;
    const _this = this;
    if (overwrite == null) {
      overwrite = false;
    }
    ...
    try {
      ...
      rendererControl = $('<td>');
      renderer = $("<select class='pvtRenderer'>").bind('change', () => refresh());
      ...
      _fn = function (c) {
        let attrElem; let btns; let checkContainer; let filterItem; let filterItemExcluded; let hasExcludedItem; let keys; let showFilterList; let triangleLink; let updateFilter; let v; let valueList; let _j; let _len1; let
          _ref2;
        keys = (function () {
          let _results;
          _results = [];
        ...
        if (keys.length > opts.menuLimit) {
          ...
          btns = $('<p>');
          btns.append($('<button>').text(opts.localeStrings.selectAll).bind('click', () => valueList.find('input').prop('checked', true)));
          btns.append($('<button>').text(opts.localeStrings.selectNone).bind('click', () => valueList.find('input').prop('checked', false)));
      ...
      tr1 = $('<tr>');
      aggregator = $("<select class='pvtAggregator'>").bind('change', () => refresh());
      ...
      refreshDelayed = function () {
        let exclusions; let natSort; let pivotUIOptions; let subopts; let unusedAttrsContainer; let
          vals;
        subopts = {
        ...
        if (opts.autoSortUnusedAttrs) {
          ...
          $(unusedAttrsContainer).children('li').sort((a, b) => natSort($(a).text(), $(b).text())).appendTo(unusedAttrsContainer);
        }
  ...
  $.fn.heatmap = function (scope) {
    let colorGen; let heatmapper; let i; let j; let numCols; let numRows; let _i; let _j; let _ref;
    const _this = this;
    if (scope == null) {
      scope = 'heatmap';
    }
    ...
    heatmapper = function (scope, color) {
      let colorFor; let forEachCell; let
        values;
      ...
      return forEachCell((x, elem) => elem.css('background-color', `#${colorFor(x)}`));
    };
    switch (scope) {
      case 'heatmap':
        ...
        break;
      case 'rowheatmap':
        for (i = _i = 0; numRows >= 0 ? _i < numRows : _i > numRows; i = numRows >= 0 ? ++_i : --_i) {
          heatmapper(`.pvtVal.row${i}`, 'red');
        }
        break;
      case 'colheatmap':
        for (j = _j = 0; numCols >= 0 ? _j < numCols : _j > numCols; j = numCols >= 0 ? ++_j : --_j) {
  ...
  $.fn.barchart = function () {
    let barcharter; let i; let numCols; let numRows; let _i; let _ref;
    const _this = this;
    _ref = this.data('dimensions'), numRows = _ref[0], numCols = _ref[1];
    barcharter = function (scope) {
      let forEachCell; let max; let scaler; let
        values;
      forEachCell = function (f) {
        return _this.find(scope).each(function () {
          let x;
