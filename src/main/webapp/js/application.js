...
if (Ajax && (Ajax != null)) {
  Ajax.Responders.register({
	  onCreate() {
      if ($('spinner') && Ajax.activeRequestCount > 0) Effect.Appear('spinner', { duration: 0.5, queue: 'end' });
	  },
	  onComplete() {
      if ($('spinner') && Ajax.activeRequestCount == 0) Effect.Fade('spinner', { duration: 0.5, queue: 'end' });
	  },
  });
}
...
function selectCombo(comboBoxElem, value) {
  if (comboBoxElem != null) {
    if (comboBoxElem.options) {
      for (let i = 0; i < comboBoxElem.options.length; i++) {
