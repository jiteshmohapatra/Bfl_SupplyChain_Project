...
(function ($) {
  const delimiter = new Array();
  const tags_callbacks = new Array();
  $.fn.doAutosize = function (o) {
	    const minWidth = $(this).data('minwidth');
	        const maxWidth = $(this).data('maxwidth');
	        let val = '';
	        const input = $(this);
	        const testSubject = $(`#${$(this).data('tester_id')}`);
  ...
  $.fn.resetAutosize = function (options) {
    ...
    const maxWidth = $(this).data('maxwidth') || options.maxInputWidth || ($(this).closest('.tagsinput').width() - options.inputPadding);
    const val = '';
    const input = $(this);
    ...
    const testerId = `${$(this).attr('id')}_autosize_tester`;
      ...
      if (value != '' && skipTag != true) {
        $('<span>').addClass('tag').append(
          ...
          $('<a>', {
            ...
            title: 'Removing tag',
            text: 'x',
          }).click(() => $(`#${id}`).removeTag(escape(value))),
  ...
  $.fn.tagsInput = function (options) {
    ...
    this.each(function () {
      ...
      let markup = `<div id="${id}_tagsinput" class="tagsinput"><div id="${id}_addTag">`;
      ...
      if (settings.interactive) {
        ...
        data.removeWithBackspace && $(data.fake_input).bind('keydown', function (event) {
          if (event.keyCode == 8 && $(this).val() == '') {
						 event.preventDefault();
						 let last_tag = $(this).closest('.tagsinput').find('.tag:last').text();
