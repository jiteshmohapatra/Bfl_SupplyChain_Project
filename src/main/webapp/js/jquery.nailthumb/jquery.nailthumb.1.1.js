...
(function ($) {
  ...
  $.fn.nailthumb = function (options) {
    const opts = $.extend({}, $.fn.nailthumb.defaults, options);
    return this.each(function () {
      const $this = $(this);
      const o = $.metadata ? $.extend({}, opts, $this.metadata()) : opts;
      thumbize($this, o);
    });
  };
  function thumbize(element, options) {
    const image = setImage(element, options);
    const container = setContainer(element, options);
  ...
  function doThumb(image, container, options) {
    ...
    let imageDims = getImageDims(image, options);
    debugObject('image', image);
    ...
    const containerDims = getContainerDims(container, options);
    debugObject('container', container);
    debugObject('containerDims', containerDims);
    const prop = getProportion(containerDims, imageDims, options);
  ...
  function setImage(element, options) {
    ...
    let finder = options.imageCustomFinder;
    ...
    if (finder) {
      let img = finder(element, options);
  ...
  function imageUrlFinder(element, options) {
    const image = $('<img />').attr('src', options.imageUrl).css('display', 'none').data('nailthumb.replaceto', element);
  ...
  function resize(image, imageDims, container, containerDims, prop, options) {
    const iw = imageDims.width * prop;
    const ih = imageDims.height * prop;
    let top = 0; let left = 0; let
      diff;
    const direction = getDirections(options.fitDirection);
  ...
  function replaceImage(image, imageDims, container, containerDims, ih, iw, left, top, diff, options) {
    const element = image.data('nailthumb.replaceto');
    const replaceto = findReplaceTo(element, options);
  ...
  function showImage(image, imageDims, container, containerDims, ih, iw, left, top, diff, options) {
    if (options.replaceAnimation == 'animate') {
      ...
      if (options.replaceAnimation == 'fade') {
        image.fadeIn(options.animationTime, options.animation, () => {
          afterAppear(image, imageDims, container, containerDims, ih, iw, left, top, diff, options);
        });
      } else if (options.replaceAnimation == 'slide') {
        image.slideDown(options.animationTime, options.animation, () => {
          afterAppear(image, imageDims, container, containerDims, ih, iw, left, top, diff, options);
        });
      } else if (options.replaceAnimation && options.replaceAnimation instanceof Function) {
        options.replaceAnimation(image, () => {
          afterAppear(image, imageDims, container, containerDims, ih, iw, left, top, diff, options);
        }, options);
  ...
  function decorate(image, imageDims, container, containerDims, ih, iw, left, top, diff, options) {
    if (options.title || (options.titleAttr && image.attr(options.titleAttr))) {
      const title = options.title ? options.title : image.attr(options.titleAttr);
      if (title) {
        const span = $(`<span class="${options.titleClass}">${title}</span>`);
        if (containerDims.innerHeight > ih) span.css('top', containerDims.innerHeight - ih);
        else span.css('top', '0px');
        container.append(span);
        let tit = getHiddenDims(span);
        const im = getHiddenDims(image);
        ...
        let outbound = containerDims.offsetTop + containerDims.innerHeight - tit.offsetTop;
        ...
        const cloneDims = getHiddenDims(clone);
        clone.remove();
        debugObject('decorate cloneDims', cloneDims);

        if (options.titleWhen == 'hover') {
          container.unbind('mouseenter mouseleave').hover(() => {
            ...
            if (options.animateTitle) {
              ...
              span.stop(true).animate({
                top: `-=${doubleDelta}`,
              }, options.titleAnimationTime, options.titleAnimation, () => {
  ...
  function scrollFunction(span, width, visibleWidth, options) {
    return function () {
      const indent = Number(span.find(`span.${options.titleScrollerClass}`).css('left').replace(/[^-\d]/g, ''));
      debug('indent', indent);
      debug('width', width);
      debug('visibleWidth', visibleWidth);
      debug('width <= -indent', (width <= -indent));
      let delta = width + indent;
      if (delta <= 0) {
  ...
  function getProportion(containerDims, imageDims, options) {
    ...
    let prop = containerDims.innerWidth / imageDims.width;
    ...
    } else if (containerDims.innerHeight / imageDims.height > prop) {
      prop = containerDims.innerHeight / imageDims.height;
    }
  ...
  function getContainerDims(container, options) {
    const containerDims = getHiddenDims(container);
  ...
  function getHiddenDims(elems) {
    let dims = null; let i = 0; let offset; let
      elem;

    while ((elem = elems[i++])) {
      const hiddenElems = $(elem).parents().andSelf().filter(':hidden');
  ...
  function getHiddenCloneDims(elems) {
    ...
    while ((elem = elems[i++])) {
      ...
      if (!hiddenElems.length) {
        ...
        const clone = hiddenElems.eq(0).clone();
        $('body').append(clone);
  ...
  $.fn.nailthumb.evalServerSideParams = function (image, container, options) {
    if (options.serverSideParams) {
      ...
      $.each(options.serverSideParams, (key, val) => {
  ...
  $.fn.nailthumb.setServerSideParams = function (image, container, options) {
    if (options.serverSideParams) {
      ...
      const pars = $.fn.nailthumb.evalServerSideParams(image, container, options);
      url += pars;
      image.attr('src', url);
    }
  };
  ...
  $.fn.nailthumb.defaults = {
    ...
    imageCustomFinder: null, /* function(element,options){            return null;
        } */
  ...
  function debug(log, jQueryobj, force) {
    try {
      if ((DEBUG && window.console && window.console.log) || force) window.console.log(`${log}: ${jQueryobj}`);
    } catch (ex) {}
  }
  function debugObject(log, jQueryobj, force) {
    try {
      ...
      debug(log, jQueryobj);
      if ((DEBUG && window.console && window.console.log) || force) window.console.debug(jQueryobj);
    } catch (ex) {}
  }
}(jQuery));
