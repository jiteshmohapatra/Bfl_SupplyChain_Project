...
(function ($) {
  ...
  function Plot(placeholder, data_, options_, plugins) {
    ...
    function parseOptions(opts) {
      ...
      let i; let axisOptions; let axisCount;
      ...
      for (const n in hooks) if (options.hooks[n] && options.hooks[n].length) hooks[n] = hooks[n].concat(options.hooks[n]);
    ...
    function fillInSeriesOptions() {
      let neededColors = series.length; let maxIndex = -1; let
        i;
      ...
      let c; const colors = []; const colorPool = options.colors;
      const colorPoolSize = colorPool.length; let
        variation = 0;
    ...
    function allocateAxisBoxFirstPhase(axis) {
      ...
      let { tickLength } = axis.options;
      let { axisMargin } = options.grid;
    ...
    function setupGrid() {
      let i; const axes = allAxes(); const
    ...
    function setTicks(axis) {
      const oticks = axis.options.ticks; let
        ticks = [];
    ...
    function extractRange(ranges, coord) {
      let axis; let from; let to; let key; const
        axes = allAxes();
    ...
    function drawGrid() {
      ...
      for (let j = 0; j < axes.length; ++j) {
        const axis = axes[j]; const { box } = axis;
        const t = axis.tickLength; var x; var y; var xoff; var
          yoff;
    ...
    function drawSeriesPoints(series) {
      ...
      const sw = series.shadowSize;
      const { radius } = series.points;
      const { symbol } = series.points;
    ...
    function drawBar(x, y, b, barLeft, barRight, fillStyleCallback, axisx, axisy, c, horizontal, lineWidth) {
      ...
      let drawLeft; let drawRight; let drawTop; let drawBottom;
      let tmp;
    ...
    function insertLegend() {
      ...
      const fragments = []; const entries = []; let rowStarted = false;
      const lf = options.legend.labelFormatter; let s; let
        label;
    ...
    function findNearbyItem(mouseX, mouseY, seriesFilter) {
      ...
      let item = null; const foundPoint = false; let i; let j; let
        ps;
      ...
      for (i = series.length - 1; i >= 0; --i) {
        ...
        if (s.bars.show && !item) { // no other point can be nearby
          ...
          for (j = 0; j < points.length; j += ps) {
            var x = points[j]; var y = points[j + 1]; const
              b = points[j + 2];
            if (x == null) continue;
