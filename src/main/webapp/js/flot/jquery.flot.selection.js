...
(function ($) {
  function init(plot) {
    ...
    function getSelection() {
      ...
      const r = {}; const c1 = selection.first; const
        c2 = selection.second;
      $.each(plot.getAxes(), (name, axis) => {
        if (axis.used) {
          const p1 = axis.c2p(c1[axis.direction]); const
    ...
    function selectionIsSane() {
      const { minSize } = plot.getOptions().selection;
    ...
    plot.hooks.drawOverlay.push((plot, ctx) => {
      ...
      if (selection.show && selectionIsSane()) {
        ...
        const c = $.color.parse(o.selection.color);
