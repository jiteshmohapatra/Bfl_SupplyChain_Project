...
(function ($) {
  $.fn.scannerDetection = function (options) {
    ...
    this.each(function () {
      const self = this; const $self = $(self); let firstCharTime = 0; let lastCharTime = 0; let stringWriting = ''; let callIsScanner = false; let testTimer = false; let
        scanButtonCounter = 0;
      ...
      $self.data('scannerDetection', { options }).unbind('.scannerDetection').bind('keydown.scannerDetection', (e) => {
        ...
        // So need that to check endChar and startChar (that is often tab or enter) and call keypress if necessary
          ...
          const e2 = jQuery.Event('keypress', e);
