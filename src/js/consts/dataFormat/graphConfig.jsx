...
function loadColorDataset(index, data, type, subtype, colorConfig) {
  ...
  if (type === "line") {
    ...
    if (colorConfig.data.colorsArray && colorConfig.data.colorsArray.length) {
      dataset.pointBackgroundColor = getArrayOfColors(
        dataset.data.length,
        colorConfig,
      );
      dataset.pointBorderColor = getArrayOfColors(
        dataset.data.length,
        colorConfig,
      );
      dataset.pointHoverBackgroundColor = getArrayOfColors(
        dataset.data.length,
        colorConfig,
        true,
      );
      dataset.pointHoverBorderColor = getArrayOfColors(
        dataset.data.length,
        colorConfig,
        true,
      );
  ...
  if (type === "horizontalBar") {
    dataset.backgroundColor = getArrayOfColors(
      dataset.data.length,
      colorConfig,
    );
    dataset.hoverBackgroundColor = getArrayOfColors(
      dataset.data.length,
      colorConfig,
      true,
    );
...
function loadGraphColors(payload) {
  ...
  for (let i = 0; i < datasets.length; i += 1) {
    ...
    if (payload.config.colors && payload.config.colors.datasets) {
      colorConfig.data.color = Object.keys(payload.config.colors.datasets).find(
        (key) =>
          payload.config.colors.datasets[key].includes(datasets[i].label),
      );
    }

    if (payload.config.colors && payload.config.colors.labels) {
      const datasetColor = colorConfig.data.color;
      colorConfig.data.colorsArray = payload.data.labels.map(
        () => datasetColor,
      );
      payload.data.labels.forEach((label, index) => {
        const labelColor = Object.keys(payload.config.colors.labels).find(
          (key) => {
            ...
            payload.config.colors.labels[key].forEach((labelConfig) => {
              if (
                ...
                if (
                  labelConfig.code === label.code &&
                  labelConfig.message === label.message
                ) {
                  response = true;
                }
              }
    ...
    legend: {
      display:
        ...
        (window.innerWidth > 1000 ||
          (window.innerWidth < 865 && window.innerWidth > 590) ||
          (sizeGraph === "big" && window.innerWidth > 500)),
      labels: {
        fontSize: 12,
        usePointStyle: true,
      },
...
function loadGraphOptions(payload) {
  ...
  if (payload.config.datalabel) {
    labelAlignment =
      payload.type === "horizontalBar" ? "horizontal" : "vertical";
