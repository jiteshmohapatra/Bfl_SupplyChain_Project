...
export const debouncePeopleFetch = (waitTime, minSearchLength) =>
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get("/api/persons", { params: { name: searchTerm, status: true } })
        .then((result) =>
          callback(
            _.map(result.data.data, (obj) => ({
              ...obj,
              value: obj.id,
              label: obj.name,
            })),
    ...
    if (searchTerm && searchTerm.length >= minSearchLength) {
      ...
      locationApi
        ...
        .then((result) =>
          callback(
            _.map(result.data.data, (obj) => {
              const locationTypeData = withTypeDescription
                ? ` [${obj.locationType.description}]`
                : "";
              ...
              return {
                ...
                label: withOrgCode
                  ? `${obj.organizationCode ? `${obj.organizationCode} - ` : ""}${label}`
                  : label,
              };
            }),
          ),
...
export const debounceGlobalSearch = (waitTime, minSearchLength) =>
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(encodeURI(`/json/globalSearch?term=${searchTerm}`))
        .then((result) =>
          callback(
...
export const debounceProductsFetch = (
  waitTime,
  minSearchLength,
  locationId,
  params = {},
  ...
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(
          encodeURI(
            `/api/products/search?name=${searchTerm}&productCode=${searchTerm}&location.id=${locationId}`,
          ),
        )
        .then((result) =>
          callback(
  ...
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(
          encodeURI(
            `/api/products/search?name=${searchTerm}&productCode=${searchTerm}&availableItems=true`,
          ),
        )
        .then((result) =>
          callback(
...
export const debounceProductsInOrders = (
  waitTime,
  minSearchLength,
  vendor,
  destination,
  ...
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(
          encodeURI(
            `/api/combinedShipmentItems/getProductsInOrders?name=${searchTerm}&vendor=${vendor}&destination=${destination}`,
          ),
        )
        .then((result) =>
          callback(
...
export const debounceOrganizationsFetch = (
  waitTime,
  minSearchLength,
  roleTypes = ["ROLE_SUPPLIER"],
  active = false,
) =>
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(
          `/api/organizations?q=${searchTerm}${roleTypes ? roleTypes.map((roleType) => `&roleType=${roleType}`).join("") : ""}&active=${active}`,
        )
        .then((result) =>
          callback(
            _.map(result.data.data, (obj) => ({
...
export const debounceLocationGroupsFetch = (waitTime, minSearchLength) =>
  _.debounce((searchTerm, callback) => {
    if (searchTerm && searchTerm.length >= minSearchLength) {
      apiClient
        .get(`/api/locationGroups?q=${searchTerm}`)
        .then((result) =>
          callback(
            _.map(result.data.data, (obj) => ({
              id: obj.id,
...
export const organizationsFetch = (
  ...
  apiClient
    .get(
      `/api/organizations?${roleTypes ? roleTypes.map((roleType) => `&roleType=${roleType}`).join("") : ""}&active=${active}`,
    )
    .then((res) => {
      if (res.data.data) {
...
export const fetchProductClassifications = async (facilityId) => {
  const { data } =
    await productClassificationApi.getProductClassifications(facilityId);
  if (data.data) {
