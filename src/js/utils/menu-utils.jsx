...
export const checkActiveSection = ({
  ...
  const matchedPath = Object.keys(menuUrls).find((section) => {
    ...
    const foundURL = menuUrls[section].find((url) => {
      ...
      if (
        pathnameWithoutParams.length > sectionPath.length &&
        pathnameWithoutParams[sectionPath.length] !== "/"
      ) {
        return false;
      }
      ...
      if (
        !pathnameWithoutParams.includes(sectionPath.replace(/\/index$/, ""))
      ) {
        return false;
      }
      ...
      if (!_.isEmpty(sectionSearch)) {
        const { direction, ...otherParams } = queryString.parse(
        ...
        if (!direction) {
          return Object.values(otherParams).every((param) =>
            sectionSearch.includes(param),
          );
        }
        return sectionSearch
          .split("&")
          .every((param) => search.includes(param));
      }
      return true;
    });
    return !!foundURL;
  ...
  const matchingFromSectionsUrlParts =
    menuSectionsUrlParts &&
    Object.keys(menuSectionsUrlParts).find(
      (sectionName) =>
        !!menuSectionsUrlParts[sectionName].some((section) =>
...
export const getAllMenuUrls = (menuConfig) =>
  Object.entries(menuConfig).reduce((acc, [, section]) => {
    if (!acc[section.label]) {
      ...
      if (section.subsections) {
        return {
          ...
          [section.id]: section.subsections.flatMap((subsection) =>
            subsection.menuItems.map((item) => item.href),
          ),
        };
      }
