...
export const renderFormFields = ({
  ...
  const filterElementClass = filterElement
    ? "filter-group"
    : "mb-0 mx-1 form-group";
  const requiredClass = required ? "required" : "";
  const hiddenClass = hidden ? "d-none" : "";
  const hasErrorClass =
    (touched || fieldTouched || showError) && error ? "has-error" : "";
  const className = [
    supplementClass,
    filterElementClass,
    requiredClass,
  ...
  return (
    <div
      className={`${!filterElement ? "mt-2" : ""} ${className}`}
      data-testid="form-field"
      aria-label={fieldAriaLabel}
    >
      <div className={`${filterElement ? "d-flex flex-wrap flex-1" : "row"}`}>
        {typeof FieldLabel === "string" ? (
          <label
            ...
            className={`${!filterElement ? "col-md-2 col-7 col-form-label col-form-label-xs text-center  text-md-right" : ""}`}
          >
            {FieldLabel && (
            ...
            {otherAttr.withTooltip && (
              <Tooltip
                ...
                html={
                  injectionData
                    ? ReactHtmlParser(
                        translate(tooltip, tooltip, injectionData),
                      )
              ...
              >
                &nbsp;
                <i
                  className="fa fa-question-circle-o text-primary"
                  aria-hidden="true"
                />
              </Tooltip>
            )}
        ...
        <div
          className={`form-element-container ${!filterElement ? "col-md-4 col-7" : "flex-1 filter-element-container"}`}
        >
          <CustomTooltip
      ...
      <div className="row" aria-label="subtext">
        ...
        <div className="help-block" style={{ float: "left" }}>
          ...
          {error && (touched || fieldTouched || showError) && (
...
export const validateDateIsSameOrAfter = (laterDate, earlierDate) => {
  ...
  return laterDateParsed
    .startOf("day")
    .isSameOrAfter(earlierDateParsed.startOf("day"));
};
