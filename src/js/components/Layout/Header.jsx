...
const Header = ({
  ...
  <div className="navbar p-0">
    ...
    <nav
      className="navbar navbar-expand-md navbar-light bg-light bg-white main-wrapper p-0 px-md-4"
      aria-label="main"
    >
    ...
    {Object.entries(bars)?.map(([bar, values]) =>
      infoBarVisibility[values.name] ? <InfoBar {...values} key={bar} /> : null,
    )}
  </div>
);
...
Header.propTypes = {
  ...
  infoBarVisibility: PropTypes.shape(
    Object.keys(InfoBar).reduce(
      (acc, bar) => ({ ...acc, [bar]: PropTypes.string }),
      {},
    ),
  ).isRequired,
};
