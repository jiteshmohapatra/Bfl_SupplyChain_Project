...
describe("hasMinimumRequiredRole()", () => {
  it("should return true if have minimum required role", () => {
    ...
    expect(minimumRequiredRole).toBeTruthy();
  });
  ...
  it("should return false if do not have minimum required role", () => {
    ...
    expect(minimumRequiredRole).toBeFalsy();
  });
...
describe("findActions()", () => {
  it("should filter by status", () => {
    ...
    const filteredByStatus = findActions(
      actionList,
      { original: { status: "test2" } },
      {
        supportedActivities: [],
        highestRole: "Admin",
      },
    ...
    expect(filteredByStatus[0].id).toBe(2);
  });
  ...
  it("should filter by minimum required role", () => {
    ...
    const filteredByMinimumRequiredRole = findActions(
      actionList,
      { original: { status: "test" } },
      {
        supportedActivities: ["testActivity"],
    ...
    expect(filteredByMinimumRequiredRole.length).toBe(2);
    expect(filteredByMinimumRequiredRole[0].id).toBe(2);
    expect(filteredByMinimumRequiredRole[1].id).toBe(3);
  });
  ...
  it("should filter using custom filters", () => {
    ...
    const customFilteredActions = findActions(
      actionList,
      { original: { status: "test" } },
      {
        supportedActivities: ["testActivity"],
    ...
    expect(customFilteredActions.length).toBe(2);
    expect(customFilteredActions[0].id).toBe(1);
    expect(customFilteredActions[1].id).toBe(3);
  });
});
...
describe("transformFilterParams()", () => {
  it("should transform normal key", () => {
    ...
    const transformedFilterParams = transformFilterParams(
      filterValues,
      filterAccessors,
    );
    expect(Object.keys(transformedFilterParams).length).toBe(1);
    expect(transformedFilterParams.origin).toBe("test");
  });
  it("should transform key which is an array", () => {
    ...
    expect(Object.keys(transformedFilterParams).length).toBe(1);
    expect(transformedFilterParams.origin.length).toBe(2);
    expect(transformedFilterParams.origin[0]).toBe("test");
    expect(transformedFilterParams.origin[1]).toBe("test2");
  });
  ...
  it("should return empty object", () => {
    ...
    expect(Object.keys(transformedFilterParams).length).toBe(0);
  });
});
...
describe("getParamList()", () => {
  ...
  it("transform string into an array", () => {
    ...
    expect(paramList.length).toBe(1);
    expect(paramList[0]).toBe("test string");
  });
});
