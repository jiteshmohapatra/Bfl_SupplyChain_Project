...
describe("getAllMenuUrls", () => {
  ...
  it("should return subsection with url", () => {
    ...
    const menuUrls = {
      Inventory: ["/openboxes/inventory/browse?resetSearch=true"],
    };
    expect(getAllMenuUrls(menuConfig)).toStrictEqual(menuUrls);
  });
...
describe("checkActiveSection", () => {
  ...
  it("should match path with params", () => {
    const path = {
      pathname: "/openboxes/order/list",
      search: "?orderType=PUTAWAY_ORDER&status=PENDING",
    };
    expect(
      checkActiveSection({
        menuUrls,
  ...
  it("should match section from config", () => {
    ...
    expect(
      checkActiveSection({
        ...
        path,
        params,
        menuSectionsUrlParts: { test: ["testPath"] },
      }),
    ).toBe("test");
  });
});
